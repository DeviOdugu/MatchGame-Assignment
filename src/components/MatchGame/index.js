...
class MatchGame extends Component {
  state = {
    ...
    initialImage: imagesList[0].imageUrl,
    isGameOver: false,
  }
  ...
  clickThumbnailImage = image => {
    const {initialImage, score} = this.state
    const randomImage = Math.floor(Math.random() * imagesList.length)

    if (initialImage === image.imageUrl) {
      this.setState({
        initialImage: imagesList[randomImage].imageUrl,
        score: score + 1,
      })
    } else {
      this.setState(
  ...
  render() {
    const {activeId, initialImage, score, isGameOver, seconds} = this.state
    ...
    return (
      <div className="bg">
        <nav className="nav-bg">
          ...
          <div className="a">
            <p className="score">
              Score: <span className="s"> {score} </span>
            </p>
            <img
        ...
        {!isGameOver && (
          <div>
            ...
            <ul className="thumbNailImages_list">
              {filteredImages.map(eachImage => (
                <ImageItem
                  key={eachImage.id}
                  imageDetails={eachImage}
                  clickThumbnailImage={this.clickThumbnailImage}
                />
              ))}
            </ul>
          </div>
